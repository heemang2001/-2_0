...
vector<string> solution(vector<string> s)
{
	...
	for (int i = 0; i < s.size(); ++i)
	{
		...
		// 2️. 스택에 남아 있는 문자들("110"이 모두 제거된 형태의 문자열)을 꺼내 문자열로 만듬과 동시에
		// count 개수의 연속된 "110" 문자열을 삽입할 위치가 될 인덱스를 찾는다. 
		// 스택에 1011 이 있는 상태라면 1->1->0->1 순으로 꺼내지게 되며 각각 str의 맨 앞에 삽입한다.
		// 결론적으로 str은 "1011" 이 되고 인덱스는 2가 된다. 
